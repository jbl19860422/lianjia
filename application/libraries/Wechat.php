...
class Wechat
{
  ...
  const MEDIA_FOREVER_BATCHGET_URL = '/material/batchget_material?';
	...
	//accesstoken相关
	private $redis;
	private $redis_connect_ret = false;
	private $redis_ip = "127.0.0.1";
	private $redis_port = 6379;
	private $passwd = 'jiangbaolin';
	private $redis_accesstoken_name = "access_token";
	...
	public function __construct($options)
	{
		...
		if(!$this->reids_connect_ret) {
			$this->redis_ip = $options['redis_ip'];
			$this->redis_port = $options['redis_port'];
			$this->redis_passwd = $options['redis_passwd'];
			$this->myappid = $options['myappid'];
			$this->redis_accesstoken_name = $this->myappid.'.access_token';
			
			$this->redis = new Redis();
			$this->redis_connect_ret = $this->redis->connect($this->redis_ip, $this->redis_port);
			$this->redis->auth($this->redis_passwd);
		}
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		if($this->redis_connect_ret) {
			log_message('error', 'key='.'wechat'.$this->myappid.$cachename);
			$this->redis->set('wechat'.$this->myappid.$cachename, $expired, $value);
		} else {
			return false;
		}
	}
	...
	protected function getCache($cachename){
		...
		if($this->redis_connect_ret) {
			return $this->redis->get('wechat'.$this->myappid.$cachename);
		}
		return false;
	}
	...
	*/
	public function getAccessToken($appid="", $appsecret="") {
		...
		if($access_token) {
			log_message('error', 'get access token from redis '.$access_token);
			return $access_token;
		}
		...
		if ($result) {
			...
			$this->setCache('access_token', 3600, $json['access_token']);
	...
	public function getMenu(){
		...
		//{"errcode":46003,"errmsg":"menu no exist hint: [fJq4Ja0190vr21]"}
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || isset($json['errcode'])) {
	 ...
	 * @return raw data
	 */
	public function downloadSaveFile($media_id,$is_video, $filename){
		...
		$result = $this->http_get($url_prefix.self::MEDIA_GET_URL.'access_token='.$access_token.'&media_id='.$media_id);
		if ($result) {
      if (is_string($result)) {
          $json = json_decode($result,true);
