...
<div id="content_warp">
	...
	<div class="mg-c">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="col-sm-12" style="font-size:1.5rem;font-weight:bolder;margin-top:20px;">
					...
					<input type="checkbox" class="checkbox-inline" v-model="isAgent" @click="onSetType(1)"></input>
					<label style="font-size:1.2rem;color:#999">代理人</label>
					<input type="checkbox" class="checkbox-inline" style="margin-left:20px" v-model="isDecider" @click="onSetType(2)"></input>
				...
				<div class="col-sm-12" style="font-size:1.5rem;font-weight:bolder;;margin-top:20px;" v-if="guest_add.entrust_type==2">
					...
					<select class="form-control" style="width:auto;display:inline;" v-model="guest_add.rent_intention">
						<option value="0">弱</option>
						<option value="1">一般</option>
						<option value="2">强烈</option>
					</select>
		...
		<div class="col-sm-12" style="font-size:2.5rem;font-weight:bolder;text-align:center;">
			<span class="btn btn-success btn-rounded btn-big" style="width:50%;font-size:2rem;" @click="editGuest()">修改</span>
		</div>
	</div>
	...
	$(function() {
		g_page = new Vue({
			...
			data:{
				...
				guest_add:<?=json_encode($guest)?>
			},
			created:function() {
				if(this.guest_add.mobiles) {
					this.guest_add.mobiles = this.guest_add.mobiles.split("|");
				} else {
					this.guest_add.mobiles = [];
				}
				for(var i = 0; i < this.guest_add.mobiles.length; i++) {
					if(this.guest_add.mobiles[i] == '') {
						this.guest_add.mobiles.splice(i,1);break;
					}
				}
				
				this.guest_add.contacts = JSON.parse(this.guest_add.contacts);
				this.guest_add.labels = this.guest_add.labels.split("|");
			},
			computed:{
				isDecider:function() {
			...
			methods:{
				...
				onSetType:function(v) {
					if(v == 1) {
						if(this.guest_add.type == 0) {
							this.guest_add.type = 1;
						} else if(this.guest_add.type == 1) {
							this.guest_add.type = 0;
						} else if(this.guest_add.type == 2) {
							this.guest_add.type = 3;
						} else if(this.guest_add.type == 3) {
							this.guest_add.type = 2;
						}
					} else if(v == 2) {
						if(this.guest_add.type == 0) {
							this.guest_add.type = 2;
				...
				editGuest:function() {
					...
					var that = this;
					API.invokeModuleCall(g_host_url,'guest','editGuest', this.guest_add, function(json) {
						if(json.code == 0) {
							showMsg('添加成功！');
							history.back(-1);
						}
