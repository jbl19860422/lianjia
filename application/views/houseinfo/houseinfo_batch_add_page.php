...
<head>
	...
	<link href="../../../static/css/base.css?v=12" rel="stylesheet">
			...
			<div class="panel-body" style="padding-right:150px">
				<div class="pro-bar">
					<div style="width:610px;height:80px;margin:auto;position:relative;">
						<div :class="{step:true,step1:true,active:curr_step==1}" @click="curr_step=1;">
							<div>
							1
							</div>
							<p class="step-info">
				...
				<form class="form-horizontal" role="form" style="margin-top:50px;margin-left:8%;" v-show="curr_step==1">
					<div class="form-group" v-for="(h,ind) in selected_houses">
						<div class="col-sm-1">
						
						</div>
						<div class="btn-group col-sm-10">
							<div class="btn btn-warning" v-html="(h.area!=null?h.area.name:'')+(h.trade_area!=null?h.trade_area.name:'')+h.community.name+h.bb.name+'栋'+h.bu.name+'单元'+h.house.floor+'楼'+h.house.name"></div>
							<div class="btn btn-danger" @click="delSelectedHouse(ind)">删除</div>
						</div>
					</div>
				...
				<form class="form-horizontal" role="form" style="margin-top:50px;margin-left:8%;" v-show="curr_step==2">
					<house-item v-for="(h,index) in selected_houses" :h="h" :ind="index" :rooms="rooms" :halls="halls" :kitchens="kitchens" :toilets="toilets" :openhome_times="openhome_times" :entrust_srcs="entrust_srcs" :employees="employees"></house-item>
				...
				<form class="form-horizontal" role="form" style="margin-top:50px;margin-left:8%;" v-show="curr_step==3">
					<div style="clear:both;height:30px;"></div>
					<div class="form-group">
						<div class="btn btn-success" style="margin-left:auto;margin-right:auto;width:60%;display:block;" @click="window.close();">关闭</div>
					</div>
				</form>
			</div>
		</div>
...
<script type="text/javascript">
	...
	var houseItem = Vue.component('house-item', {
		props:['h','ind', 'rooms', 'halls', 'kitchens', 'toilets', 'openhome_times', 'entrust_srcs', 'employees'],
		template:	'<div style="width:49%;float:left;border:1px solid #bbb;border-bottom-left-radius:5px;border-bottom-right-radius:5px;margin-right:5px;" class="panel panel-default">'+
						'<div class="panel-heading panel-title">'+
							'<div><p class="panel-p" v-html="(h.area!=null?h.area.name:\'\')+(h.trade_area!=null?h.trade_area.name:\'\')+h.community.name+h.bb.name+\'栋\'+h.bu.name+\'单元\'+h.house.floor+\'楼\'+h.house.name"></p>'+
							'<i :class="{fa:true,\'fa-angle-double-up\':!h.collapsed,\'fa-angle-double-down\':h.collapsed}" style="float:right;font-size: 2.4rem; font-weight: bolder; cursor: pointer;" @click="h.collapsed=!h.collapsed;$(\'#hi-\'+ind).slideToggle()"></i>'+
							'</div>'+
						'</div>'+
						'<div class="panel-body" :id="\'hi-\'+ind">'+
							'<div class="form-group">'+
								'<div class="col-sm-3">'+
								...
								'<div class="col-sm-9" style="display:flex">'+
									'<div class="form-group" style="padding-left: 0px; margin-left: -43px; margin-top: -8px; position: relative; padding-right: 0px;">'+
										'<select v-model="h.house_info_add.room_count" class="form-control" style="display:inline;width:auto">'+
											'<option v-for="c in rooms" :value="c" v-html="c"></option>'+
										'</select>'+
										'<span>室</span>'+
										'<select v-model="h.house_info_add.hall_count" class="form-control" style="display:inline;width:auto;margin-left:10px;">'+
											'<option v-for="c in halls" :value="c" v-html="c"></option>'+
										'</select>'+
										'<span>厅</span>'+
										'<select v-model="h.house_info_add.kitchen_count" class="form-control" style="display:inline;width:auto;margin-left:10px;">'+
											'<option v-for="c in kitchens" :value="c" v-html="c"></option>'+
										'</select>'+
										'<span>厨</span>'+
										'<select v-model="h.house_info_add.toilet_count" class="form-control" style="display:inline;width:auto;margin-left:10px;">'+
							...
							'<div class="form-group">'+
								...
								'</div>'+
								'<div class="col-sm-9" style="display: flex; padding-left: 0px; margin-left: -30px; margin-top: -8px; position: relative; padding-right: 0px;">'+
									'<input type="text" class="form-control" v-model="h.house_info_add.area" style="display:inline;width:auto"></input><span>㎡</span>'+
								'</div>'+
							'</div>'+
							...
							'<div class="form-group">'+
								...
								'</div>'+
								'<div class="col-sm-9 checkbox" style="display: flex; padding-left: 0px; margin-left: -30px; margin-top: -8px; position: relative; padding-right: 0px;">'+
							...
							'<div class="form-group">'+
								'<div class="col-sm-3">'+
									'<label for="name" v-show="h.house_info_add.entrust_type==1">售&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：</label>'+
									'<label for="name" v-show="h.house_info_add.entrust_type==2">租&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;价：</label>'+
								'</div>'+
								'<div class="col-sm-9" style="display: flex; padding-left: 0px; margin-left: -30px; margin-top: -8px; position: relative; padding-right: 0px;">'+
									'<input type="tel" class="form-control" v-model="h.house_info_add.price" style="display:inline;width:auto"></input><label style="margin-left:5px;margin-top:8px" v-html="h.house_info_add.entrust_type==1?\'万\':\'元/月\'">万</label>'+
								'</div>'+
							'</div>'+
							...
							'<div class="form-group">'+
								...
								'</div>'+
								'<div class="col-sm-9 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;">'+
									'<select v-model="h.house_info_add.openhome_time" class="form-control">'+
							...
							'<div class="form-group">'+
								...
								'</div>'+
								'<div class="col-sm-9 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;">'+
									'<select v-model="h.house_info_add.entrust_src1"  class="form-control">'+
							...
							'<div class="form-group" style="margin-bottom:0px">'+
								...
								'</div>'+
								'<div class="col-sm-9 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;">'+
									'<input type="text" class="form-control" v-model="h.curr_maintainer_name"></input>'+
									'<div style="background-color:#fff;left:100px;position:absolute;top:34px;left:0px;text-align:left;border-left:1px solid #eee;border-right:1px solid #eee;border-bottom:1px solid #eee;width:100%;z-index:100;" class="search-communities" v-show="!h.change_by_user">'+
										'<ul style="margin-bottom:0px">'+
											'<li v-for="f in h.filter_employees" class="c-name" style="height:35px;line-height:35px;color:#333;font-size:15px;padding-left:10px;margin-left:0px;" v-html="f.name+\'(\'+f.work_no+\')\'" @click="selMaintainer(f)"></li>'+
										'</ul>'+
									'</div>'+
								'</div>'+
							'</div>'+
							...
							'<div class="form-group step1">'+
								...
								'<div class="col-sm-2 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;margin-right:10px;">'+
									'<input type="text" class="form-control" v-model="h.house_info_add.owner.name"></input>'+
								'</div>'+
								...
								'</div>'+
								'<div class="col-sm-2 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;;margin-right:10px;">'+
									'<input type="text" class="form-control" v-model="h.house_info_add.owner.wechat_num"></input>'+
								'</div>'+
								...
								'</div>'+
								'<div class="col-sm-2 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;;margin-right:10px;">'+
									'<input type="text" class="form-control" v-model="h.house_info_add.owner.mobile"></input>'+
								'</div>'+
								...
								'</div>'+
								'<div class="col-sm-2 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;;margin-right:10px;">'+
									'<input type="text" class="form-control" v-model="h.house_info_add.owner.telephone"></input>'+
								'</div>'+
							...
							'</div>'+
							'<div class="form-group" v-for="(c,index) in h.house_info_add.owner.more_contact" style="margin-bottom:0px">'+
								'<div class="col-sm-3"></div>'+
								'<div class="col-sm-9 form-group" style="display:flex;padding-left:0px;margin-left:-30px;margin-top:-8px;position:relative;padding-right:0px;">'+
									'<input type="text" class="form-control" style="width:auto;display:inline;margin-right:10px;" v-model="h.house_info_add.owner.more_contact[index]" placeholder="请输入联系方式"></input><label style="margin-right:30px;line-height:32px;cursor:pointer;color:#FF6863" @click="delMoreContact(index)">删除</label>'+
								'</div>'+
							'</div>'+
							'<div class="form-group" style="margin-bottom:0px">'+
								'<div class="col-sm-3"></div>'+
							...
							'<div class="form-group step1" v-for="(contact,index) in h.house_info_add.contacts">'+
								'<div class="col-sm-3">'+
		...
		methods:{
			selMaintainer:function(employee) {
				this.h.change_by_user = true;
				this.h.house_info_add.maintain_employee_id = employee.employee_id;
				this.h.curr_maintainer_name = employee.name+'('+employee.work_no+')';
				this.h.filter_employees = [];
			},
			delMoreContact:function(index) {
				this.h.house_info_add.owner.more_contact.splice(index,1);
			},
			delContact:function(index) {
				this.h.house_info_add.contacts.splice(index,1);
			},
			setEntrustTye:function(t) {
				this.h.house_info_add.entrust_type = t;
			},
			onClickOrientation:function( o) {
				var s = this.h.house_info_add.orientation.split("|");
				var find = false;
				for(var i = 0; i < s.length; i++) {
					if(s[i] == o) {
						find = true;
						s.splice(i,1);
						this.h.house_info_add.orientation = s.join("|");
			...
			addContacts:function() {
				if(this.h.house_info_add.contacts.length >= 2) {
					return;
				}
				this.h.house_info_add.contacts.push({
					name:'',
					wechat_num:'',
					mobile:'',
					telephone:'',
			...
			addOwnerContact:function() {
				if(this.h.house_info_add.owner.more_contact.length >= 2) {
					return;
				}
				this.h.house_info_add.owner.more_contact.push("");
			}
		},
		watch:{
			'h.curr_maintainer_name':function(val) {
				this.h.filter_employees = [];
				if(this.h.change_by_user) {
					this.h.change_by_user = false;
					this.$forceUpdate();
					return;
				}
					...
					for(var i = 0; i < this.employees.length; i++) {
						if(this.employees[i].retriving_info) {
							if(this.employees[i].retriving_info.indexOf(val) >= 0 || this.employees[i].name.indexOf(val) >= 0) {
								this.h.filter_employees.push(this.employees[i]);
							}
						}
					}
				}
		...
		g_page = new Vue({
			...
			watch:{
				curr_house_id:function(val) {
					var that = this;
					for(var i = 0; i < that.selected_houses.length; i++) {
						if(that.selected_houses[i].house.house_id == val) {
							showMsg('已经添加');
							return;
						}
					}
					API.invokeModuleCall(g_host_url,'houseinfo','queryHouseAddress2', {house_id:val}, function(json) {
						if(json.code == 0) {
							if(json.exists) {
								showMsg('该房已经在租售');
								return;
							...
							var top_floor = 1;
							if(parseInt(json.bb.max_floor) == json.house.floor) {
								top_floor = 1;
							} else {
								top_floor = 0;
							}
							
							that.selected_houses.push(
								{collapsed:false,house:json.house,area:json.area,trade_area:json.trade_area,community:json.community,bu:json.bu, bb:json.bb,
									house_info_add:{
										area_id:json.area?json.area.area_id:0,
										house_id:json.house.house_id,
										room_count:1,
										hall_count:1,
										top_floor:top_floor,
										kitchen_count:1,
										max_floor:json.bb.max_floor,
										community_id:json.community.community_id,
										community_name:json.community.name,
										toilet_count:1,
										bb_id:json.bb.bb_id,
										build_block:json.bb.name,
										bu_id:json.bu.bu_id,
										floor:json.house.floor,
										entrust_type:1,
										area:-1,
										price:1,
										orientation:'',
			...
			methods:{
				delSelectedHouse:function(ind) {
					this.selected_houses.splice(ind,1);
				},
				selMaintainer:function(h, employee) {
					this.change_by_user = true;
					h.house_info_add.maintain_employee_id = employee.employee_id;
					h.curr_maintainer_name = employee.name+'('+employee.work_no+')';
					h.filter_employees = [];
				},
				...
				addHouseInfo:function() {
					...
					for(var i = 0; i < this.communities.length; i++) {
						if(this.communities[i].community_id == this.curr_community_id) {
							area_id = this.communities[i].area_id;
							break;
						}
					}
					
					
					for(var i = 0; i < this.selected_houses.length; i++) {
						this.selected_houses[i].house_info_add.area_id = area_id;
						house_infoes.push(this.selected_houses[i].house_info_add);
					}
					this.show_mask = true;
					API.invokeModuleCall(g_host_url,'houseinfo','addHouseInfoes', {house_infoes:house_infoes}, function(json) {
						if(json.code == 0) {
							that.curr_step = 3;
				...
				goStep:function(step) {
					...
					if(step == 2) {//判断第一步参数是否正确
						if(this.selected_houses.length <= 0) {
							showMsg('您还未选择房源');
							return;
						}
