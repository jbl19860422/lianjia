$(function() {
/****************轮播图组件--编辑************/
	var carouselComPanel = Vue.component('carousel-com-panel', {
		...
		'<div class="col-md-12 col-lg-12 col-sm-12">'+
			...
			'<div class="tab-content" style="background-color:#f6f6f6;border-bottom-left-radius:10px;border-bottom-right-radius:10px;">'+
				'<carousel-com-panel1 :prop="prop" :pages="pages" :page_groups="page_groups" :acts="acts"></carousel-com-panel1>'+
				'<carousel-com-panel2 :prop="prop"></carousel-com-panel2>'+
			'</div>'+
			'<div style="clear:both;"></div>'+
		'</div>'
	});
	/****************轮播图组件--编辑1******************/
	var carouselComPanel1 = Vue.component('carousel-com-panel1',{
		props:['prop','pages','page_groups','acts'],
		template:
      '<div class="tab-pane active" :id="\'ID_editPanel1_\'+prop.id" style="background-color:#f6f6f6;text-align:left;font-size:20px;">'+
      	'<div style="margin-top:3px;text-align:center;">'+
      		'<div style="width:80%;height:100%;font-size:20px;color:white;background-color:#efbf1f;text-align:center;line-height:40px;margin-left:auto;margin-right:auto;cursor:pointer;border-radius:8px;" class="add-carousel-item">添加轮播图</div>'+
      		'<div style="display:none" id="ID_uploadCarouselImg"></div>'+
      	'</div>'+
      	'<ul style="width:80%;margin-left:auto;margin-right:auto;margin-top:20px;">'+
      		'<li v-for="(item,index) in prop.slideList" style="margin-left:auto;margin-right:auto;margin-top:20px;" class="slide-item">'+
      			'<div>'+
    ...
    mounted:function() {
    	...
    	that.curr_index = 0;
	    //添加轮播图
	    $(this.$el).find(".add-carousel-item").click(function() {
	    	that.prop.slideList.push(
	    		{
			      "clickUrl": "#",
			      "desc": "hxrj",
    ...
    updated:function() {
    	...
    	$(this.$el).find(".upload-btn").each(function(index, item) {
	    	...
	    	$(item).click(function() {
	    		...
	    		$("#ID_uploadCarouselImg").trigger('click');
	    	});
	    });
    },
    methods:{
      		...
      		'<div style="margin-top:20px">'+
      			...
      			'<div style="font-size:20px;margin-left:0px;">'+
							...
							'<div style="float:left;margin-left:20px" class="position-btns">'+
								'<span align="left" :class="{\'active\':prop.figureAlign==\'left\'}">置左</span>'+
								'<span align="center" :class="{\'active\':prop.figureAlign==\'center\'}">居中</span>'+
								'<span align="right" :class="{\'active\':prop.figureAlign==\'right\'}">置右</span>'+
							'</div>'+
						'</div>'+
    ...
    mounted:function() {
    	...
    	$('.input-interval').my_spinner({change:function(val) {
			...
			$(this.$el).find(".figure-color-input").spectrum({
    			...
    			cancelText: "取消",
    			change:function(val) {
    				that.prop.figureColor['background-color'] = "rgba("+val._r+","+val._g+","+val._b+","+val._a+")";;
    			},
    			hide:function(val) {
    				that.prop.figureColor['background-color'] = "rgba("+val._r+","+val._g+","+val._b+","+val._a+")";;
    			}
