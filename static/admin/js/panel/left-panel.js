	...
	var pageItem = Vue.component('page-item', {
		...
		methods:{
			onClickPage:function() {
				//alert(g_host_url+"/admin/page/page_manager_page?page_id="+this.page.page_id);
				//return;
				location.href=g_host_url+"/admin/page/page_manager_page?page_id="+this.page.page_id;
			},
			...
			confirmEdit:function() {
				var that = this;
				API.invokeModuleCall(_host_url,'admin','updatePageName', this.page, function(json) {
					if(json.code == 0) {
						that.edit = false;
					}
				});
			...
			copyPage:function() {
				...
				if(this.page.debug_page_conf_file != "") {
					$LAB.script(this.page.debug_page_conf_file+"?v="+this.page.debug_page_version).wait(function() {
						...
						API.invokeModuleCall(_host_url,'admin','copyPage',{page:that.page, page_conf:page_conf}, function(json) {
								if(json.code == 0) {
			...
			delPage:function() {
				...
				if(confirm('确定删除?')) {
					API.invokeModuleCall(_host_url,'admin','delPage', this.page, function(json) {
						if(json.code == 0) {
	...
	var pageGroupItem = Vue.component('page-group-item',{
			...
			methods:{
				...
				delPageGroup:function() {
					API.invokeModuleCall(_host_url,"admin",'delPageGroup', this.page_group, function(json) {
						if(json.code == 0) {
							alert('删除成功');
							location.reload();
						}
				...
				confirmEdit:function() {
					var that = this;
					API.invokeModuleCall(_host_url,'admin','updatePageGroupName', this.page_group, function(json) {
						if(json.code == 0) {
							that.edit = false;
						}
					});
