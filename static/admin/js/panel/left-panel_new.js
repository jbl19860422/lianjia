$(function() {
	...
	var pageItem = Vue.component('page-item', {
		...
		methods:{
			...
			copyPage:function() {
				var that = this;
				if(this.page.page_conf_file != "") {
					$LAB.script(this.page.page_conf_file+"?v="+this.page.page_version).wait(function() {
						var page_conf;
						...
						API.invokeCall('copyPage',{group_id:that.page.group_id,page_name:that.page.page_name+"_copy", page_conf:page_conf}, function(json) {
								if(json.code == 0) {
	...
	var page_panel = new Vue({
		...
		methods:{
			moreCom:function() {
				var testCom = Vue.component('test-com', {
					props:['prop','curr_com_id', 'com'],
					template: '<div :id="prop.id" :class="{\'test-com\':true,\'phone-component\':true}" @click="onClick($event)" >'+
			...
			addCom:function(c_type) {
				if(c_type == 'text-com') {
					...
					addBtnCom();
				} else if(c_type == 'title') {
					addTitleCom();
				} else if(c_type == 'carousel') {
					addCarouselCom();
				} else if(c_type == 'category') {
					addCategoryCom();
				} else if(c_type == 'pic-list') {
					addPicListCom();
				} else if(c_type == 'pic-txt-set') {
					addPicTxtSetCom();
