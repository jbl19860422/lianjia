$(function(){
	/*************文本组件--编辑************/
	var textComPanel = Vue.component('text-com-panel', {
		props:['prop', 'pages', 'page_groups','acts'],
		template:
		'<div class="col-md-12 col-lg-12 col-sm-12">'+
			'<ul class="nav nav-tabs">'+
				'<li class="active" style="width:50%;">'+
					'<a data-tab :href="\'#ID_editPanel1_\'+prop.id" ><i class="icon  icon-book"></i>文本组件</a>'+
				'</li>'+
				'<li style="width:50%">'+
			...
			'<div class="tab-content" style="background-color:#f6f6f6;border-bottom-left-radius:10px;border-bottom-right-radius:10px;">'+
				'<text-com-panel1 :prop="prop" :pages="pages" :page_groups="page_groups" :acts="acts"></text-com-panel1>'+
				'<text-com-panel2 :prop="prop"></text-com-panel2>'+
			'</div>'+
			'<div style="clear:both;"></div>'+
		'</div>'
	});
	/*************文本组件--编辑1**************/
	var textComPanel1 = Vue.component('text-com-panel1',{
		props:['prop','pages', 'page_groups','acts'],
		template:
      '<div class="tab-pane active" :id="\'ID_editPanel1_\'+prop.id" style="background-color:#f6f6f6;text-align:left;">'+
      	'<font-setting-pane :fontStyle="prop.styles.fontStyle" :prop="prop" :alignStyle="prop.styles.alignStyle"></font-setting-pane>'+
      	'<textarea class="text-content-area" v-model="prop.text"></textarea>'+
    ...
    mounted:function() {
    	...
    	var that = this;
    	//功能选择初始化
    	var actSelectEl = $("#ID_editPanel1_"+that.prop.id+" .page-setting-opt");
    	for(var i = 0; i < g_page_groups.length; i++) {
    		var html = "";
    		html += '<optgroup label="'+g_page_groups[i].group_name+'">';
    		for(var j = 0; j < g_pages.length;j++) {
    			if(g_pages[j].group_id == g_page_groups[i].group_id) {
    				html += '<option value="'+g_pages[j].page_id+'">'+g_pages[j].page_name+'</option>';
    			}
    		}
    		html += '</optgroup>';
    		actSelectEl.append(html);
    	}
    	actSelectEl.change(function() {
    		if($(this).val() == "prePage") {
    			that.prop.action.subtype = "prePage";
    		} else if($(this).val() === "custom-link") {
    			that.prop.action.subtype = "custom-link";
    			that.prop.action.detail = {link:"http://baidu.com"};
    		} else {
    			that.prop.action.subtype = "inner-page";
    			that.prop.action.detail = {"page_id":$(this).val()};
    		}
    	});
    },
    methods:{
	...
	/*************文本组件--编辑2************/
	var textComPanel2 = Vue.component('text-com-panel2',{
			props:['prop'],
			template:
	      '<div class="tab-pane" :id="\'ID_editPanel2_\'+prop.id" style="background-color:#f6f6f6;text-align:left;">'+
	      	'<div>'+
		      	...
		      	'<div id="ID_com_setting" class="" style="border:1px solid rgb(218, 218, 218);border-bottom-left-radius: 10px;border-bottom-right-radius: 10px;border-top:none">'+
		      		'<div>'+
			      		'<div style="padding-top: 10px; margin-left: 10px; font-size: 20px;"><div style="float:left">行高</div>'+
				      		'<div class="my-spinner input-line-height" style="float:left;margin-left:10px;">'+
										'<div class="input-area">'+
											'<input style="font-size:20px" :value="prop.styles.lhStyle[\'line-height\'].substr(0,prop.styles.lhStyle[\'line-height\'].length-2)"></input>'+
			      		...
			      		'<div style="padding-top: 10px; margin-left: 10px; font-size: 20px;">'+
			      			...
			      			'<div class="my-spinner input-border-width" style="float:left;margin-left:10px;">'+
								...
								'<div style="clear:both;margin-top: 10px; margin-left: 10px; font-size: 20px;">'+
									...
									'</div>'+
								'</div>'+
								'<div style="float:left;margin-left:20px;">圆角</div>'+
	    ...
	    mounted:function() {
	    	var that = this;
	    	$(this.$el).find(".input-line-height").my_spinner({change:function(val) {
					that.prop.styles.lhStyle['line-height'] = val+'px';
				},defaultValue:that.prop.styles.lhStyle['line-height'].substr(0,that.prop.styles.lhStyle['line-height'].length-2)});
    			...
    			change:function(val) {
    				if(val) {
    					...
    					that.prop.styles.borderStyle['border-color'] = 'transparent';
    				}
    			},
    			hide:function(val) {
    				if(val) {
